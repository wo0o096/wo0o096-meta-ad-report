...
> 이 문서와 루틴 지시문이 충돌하면 **이 문서가 우선**한다. (최종 수정: 2026-08-10)
...
### 이모지 사용 원칙 (중요 — 링크 깨짐 방지)
- **URL과 같은 줄에, 또는 URL 바로 앞/뒤에 이모지(유니코드 이모지·`:shortcode:` 모두)를 절대 붙이지 않는다.** 과거 발송에서 URL 앞의 이모지가 링크 인식을 깨뜨려 클릭 시 정상 작동하지 않는 사고가 있었다. GitHub Pages 링크는 **자신만의 줄에 이모지·부가 텍스트 없이 URL만** 단독으로 둔다.
- 그 외에도 메시지 전반에서 이모지는 꼭 필요한 곳(헤더 식별용 📊, 게임별 구분용 게임 이모지)에만 최소한으로 사용하고, 하이라이트 불릿·안내 문구 등 장식성 이모지는 지양한다.
...
• 관심 필요: 게임명 — 채널별 ROAS와 지출, 점검 제안 한 줄
...
- URL은 반드시 위 예시처럼 별도 줄에 단독으로 배치한다 (앞줄의 "전체 보고서 보기:"와 URL 사이 줄바꿈 필수, 같은 줄에 이모지·문구를 덧붙이지 않음).
...
| URL 단독 줄 배치, 이모지 최소화 | 과거 발송에서 URL 앞 이모지가 링크를 깨뜨려 클릭 불가 사고 발생. 재발 방지 |
